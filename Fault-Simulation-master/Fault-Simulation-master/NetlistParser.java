...
public class NetlistParser {
    ...
    public static void main(String[] args) {
        ...
        //setting the primary inputs values
        primaryInputs.get("1").setLineValue(0);
        primaryInputs.get("2").setLineValue(1);
        primaryInputs.get("3").setLineValue(0);
        primaryInputs.get("6").setLineValue(0);
        primaryInputs.get("7").setLineValue(0);

        //creating the circuit, computing, and printing the primary outputs values
        Circuit circuit = new Circuit(primaryInputs, primaryOutputs, gates, lines);
        circuit.compute("none");
        ArrayList<Integer> trueOutput = circuit.getOutput();
        circuit.compute("2-SA0");
        ArrayList<Integer> faultyOutput = circuit.getOutput();
        System.out.println("True primary outputs: " + trueOutput);
        System.out.println("Faulty primary outputs: " + faultyOutput);

        boolean areEqual = trueOutput.equals(faultyOutput);
        System.out.println("The fault " + (areEqual? "cannot": "can") + " be detected");
        //System.out.println(lines.get("22").getLineValue());
        //System.out.println(lines.get("23").getLineValue());
    }
}
